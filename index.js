...
function buildPackageJSON(vars, json, basePackageJSON) {
  basePackageJSON = basePackageJSON || JSON.parse(fs.readFileSync(path.join(__dirname, 'package.json'), { encoding }))
  json = json || basePackageJSON
  delete basePackageJSON.scripts;
  delete basePackageJSON.private;
  delete basePackageJSON.dependencies;
  for(var k in json) {
    var v = json[k];
    switch (typeof v) {
    case 'object':
      json[k] = buildPackageJSON(vars, v, basePackageJSON);
      break;
    case 'string':
      json[k] = v.replace(basePackageJSON.name, vars.theme.packageName);
      break;
    default:
      json[k] = v;
      break;
